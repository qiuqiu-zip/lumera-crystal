        ...
        *,
        ...
        if search:
            ...
            where_clauses.append(
                (ShopOrder.order_no.ilike(keyword)) | (ShopUser.name.ilike(keyword)) | (ShopUser.email.ilike(keyword))
            )

        if tab == "pending":
            where_clauses.append(ShopOrder.status == "created")
            where_clauses.append(ShopOrder.payment_status == "pending")
        elif tab == "processing":
            where_clauses.append(ShopOrder.status == "created")
            where_clauses.append(ShopOrder.payment_status == "paid")
        elif tab == "completed":
            where_clauses.append(ShopOrder.status == "fulfilled")
        elif tab == "cancelled":
            where_clauses.append(ShopOrder.status == "cancelled")

        where_expr = and_(*where_clauses) if where_clauses else None

        total_stmt = select(func.count()).select_from(ShopOrder).join(ShopUser, ShopOrder.user_id == ShopUser.id)
        ...
        stmt = (
            select(ShopOrder)
            .join(ShopUser, ShopOrder.user_id == ShopUser.id)
            .options(selectinload(ShopOrder.user), selectinload(ShopOrder.items), selectinload(ShopOrder.payments))
            .order_by(desc(ShopOrder.created_at))
            .offset((page - 1) * page_size)
            .limit(page_size)
        )
