...
export function CartCheckout() {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-3xl border border-mist/70 bg-gradient-to-br from-white to-ivory p-4 shadow-[0_10px_35px_-24px_rgba(39,37,34,0.65)] sm:p-5">
      ...
      <div className="space-y-5 sm:space-y-6">
        <section className="rounded-3xl border border-mist/70 bg-white/90 p-4 shadow-[0_12px_30px_-24px_rgba(32,30,27,0.7)] sm:p-5">
          ...
          <div className="space-y-3">
            {cart.map((line) => (
              ...
              >
                ...
                <input
                  className="h-10 w-20 rounded-lg border border-mist bg-white px-2 py-1 text-sm"
                  type="number"
                  min={1}
                  value={line.quantity}
                ...
                <button
                  ...
                  className="min-h-10 rounded-lg border border-stone-300 px-3 py-1.5 text-xs"
                >
                  移除
                </button>
              </div>
          ...
          <div className="mt-4 flex flex-col gap-3 rounded-2xl border border-mist/70 bg-ivory/50 p-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button
              ...
              className="min-h-11 w-full rounded-xl bg-ink px-4 py-2.5 text-sm font-medium text-ivory disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto"
            >
              {busy ? "处理中..." : "提交订单"}
            </button>
          </div>
        ...
        <section className="rounded-3xl border border-mist/70 bg-white/90 p-4 shadow-[0_12px_30px_-24px_rgba(32,30,27,0.7)] sm:p-5">
          <h2 className="mb-3 text-base font-semibold text-ink">待支付订单</h2>
            <div className="mb-3 grid gap-3 md:grid-cols-3">
              <select className="min-h-11 rounded-lg border border-mist px-3 py-2 text-sm" value={paymentMethod} onChange={(e) => setPaymentMethod(e.target.value as ShopPaymentMethod)}>
                <option value="wechat_pay">微信支付</option>
                <option value="alipay">支付宝</option>
                <option value="bank_card">银行卡</option>
                <option value="mock">测试通道</option>
              </select>
              <input className="min-h-11 rounded-lg border border-mist px-3 py-2 text-sm md:col-span-2" value={payerName} onChange={(e) => setPayerName(e.target.value)} placeholder="付款人（可选）" />
              <input className="min-h-11 rounded-lg border border-mist px-3 py-2 text-sm md:col-span-3" value={payCouponCode} onChange={(e) => setPayCouponCode(e.target.value)} placeholder="优惠券代码（支付时使用）" />
            </div>
              ...
              {pendingOrders.map((order) => (
                <div key={order.id} className="rounded-2xl border border-mist/80 bg-ivory/40 p-3">
                  ...
                  <button
                    ...
                    disabled={busy}
                    className="mt-2 min-h-10 rounded-lg border border-stone-300 px-3 py-1.5 text-xs disabled:opacity-50"
                  >
                    立即支付
                  </button>
                </div>
      ...
      {payNowOrder ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-ink/45 p-4">
          <div className="w-full max-w-lg rounded-3xl border border-mist/70 bg-white p-4 shadow-[0_20px_50px_-30px_rgba(20,20,20,0.95)] sm:p-5">
            ...
            <div className="mt-4 flex flex-col-reverse gap-2 sm:flex-row sm:items-center sm:justify-end">
              <button
                type="button"
                onClick={() => setPayNowOrder(null)}
                className="min-h-11 rounded-xl border border-stone-300 px-4 py-2 text-sm text-ink"
              >
                稍后支付
              </button>
              <button
                ...
                disabled={busy}
                className="min-h-11 rounded-xl bg-ink px-4 py-2 text-sm font-medium text-ivory disabled:opacity-50"
              >
                {busy ? "支付处理中..." : "立即支付"}
              </button>
            </div>
