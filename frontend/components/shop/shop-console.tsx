...
export function ShopConsole() {
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-3xl border border-mist/70 bg-gradient-to-br from-white to-ivory p-4 shadow-[0_14px_34px_-26px_rgba(28,28,28,0.9)] sm:p-5">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="grid w-full gap-2 sm:w-auto sm:grid-cols-[1fr_1fr_auto]">
            <input
              className="min-h-11 rounded-lg border border-mist px-3 py-2 text-sm"
              value={name}
            ...
            <button
              ...
              disabled={busy}
              className="min-h-11 rounded-lg bg-ink px-4 py-2 text-sm text-ivory disabled:opacity-60"
            >
              {busy ? "处理中..." : "刷新订单"}
            </button>
          </div>
      ...
      <section className="rounded-3xl border border-mist/70 bg-gradient-to-br from-white to-ivory p-4 shadow-[0_14px_34px_-26px_rgba(28,28,28,0.9)] sm:p-5">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="grid w-full grid-cols-3 gap-2 text-xs sm:w-auto sm:flex sm:items-center">
            <button
              type="button"
              onClick={() => setOrderFilter("all")}
              className={`min-h-9 rounded-full px-2.5 py-1 ${orderFilter === "all" ? "bg-ink text-ivory" : "border border-mist text-ink/70"}`}
            >
              全部
            </button>
            <button
              ...
              className={`min-h-9 rounded-full px-2.5 py-1 ${orderFilter === "pending" ? "bg-ink text-ivory" : "border border-mist text-ink/70"}`}
            >
              待支付
            </button>
            <button
              ...
              className={`min-h-9 rounded-full px-2.5 py-1 ${orderFilter === "paid" ? "bg-ink text-ivory" : "border border-mist text-ink/70"}`}
            >
              已支付
            </button>
          </div>
              ...
              {order.payment_status !== "paid" ? (
                <div className="mt-2 space-y-2">
                  {payingOrderId === order.id ? (
                    <div className="grid gap-2 rounded-lg border border-mist p-2 md:grid-cols-2">
                      <select
                        className="min-h-10 rounded-lg border border-mist px-2 py-1 text-xs"
                        value={paymentMethod}
                      ...
                      />
                      <input
                        className="min-h-10 rounded-lg border border-mist px-2 py-1 text-xs md:col-span-2"
                        value={payCouponCode}
                ...
                <div className="mt-2 space-y-2">
                  ...
                  <button
                    ...
                    className="min-h-10 rounded-lg border border-stone-300 px-3 py-1.5 text-xs disabled:opacity-50"
                  >
