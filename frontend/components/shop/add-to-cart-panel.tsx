  ...
  return (
    <div className="space-y-3 rounded-2xl border border-mist/70 bg-gradient-to-br from-ivory/80 to-white p-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <input
          id="detail-qty"
          className="h-10 w-24 rounded-lg border border-mist px-2 py-1 text-sm"
          type="number"
          min={1}
      ...
      <div className="grid grid-cols-1 gap-2 sm:flex sm:flex-wrap sm:items-center">
        <button
          ...
          onClick={handleAdd}
          disabled={!canBuy}
          className="min-h-11 rounded-lg bg-ink px-4 py-2 text-sm text-ivory shadow-[0_10px_24px_-16px_rgba(34,34,34,0.9)] disabled:cursor-not-allowed disabled:opacity-50"
        >
          加入购物车
        </button>
        <button
          ...
          disabled={!canBuy}
          className="min-h-11 rounded-lg border border-ink px-4 py-2 text-sm text-ink disabled:cursor-not-allowed disabled:opacity-50"
        >
          立即购买
        </button>
        <Link href="/cart" className="inline-flex min-h-11 items-center justify-center rounded-lg border border-stone-300 px-4 py-2 text-sm text-ink">
          去下单
        </Link>
      </div>
