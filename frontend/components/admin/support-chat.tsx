...
export function SupportChat() {
  ...
  return (
    <div className="rounded-3xl border border-stone-200 bg-white/80 p-4 shadow-sm sm:p-6">
      <div className="flex flex-col items-start justify-between gap-3 sm:flex-row sm:gap-6">
        <div>
          ...
          <h2 className="mt-2 text-xl font-semibold text-stone-900 sm:text-2xl">客服对话框</h2>
      ...
      <div className="mt-6 space-y-4">
        <div className="flex flex-wrap gap-2">
          {quickPrompts.map((prompt) => (
            <button
              ...
              className="rounded-full border border-stone-200 bg-white px-3 py-1.5 text-xs text-stone-600 hover:border-stone-300 hover:text-stone-900"
            >
              {prompt}
            </button>
          ))}
        ...
        <div className="max-h-[420px] space-y-3 overflow-y-auto rounded-2xl border border-stone-200 bg-[#fbfaf8] p-3 sm:p-4">
          {history.map((item) => (
            <div
              key={item.id}
              className={cn(
                ...
                item.role === "operator"
                  ? "ml-auto max-w-[90%] bg-stone-900 text-white sm:max-w-[80%]"
                  : "mr-auto max-w-[90%] bg-white text-stone-800 shadow-sm sm:max-w-[80%]"
              )}
            >
        ...
        <div className="space-y-2">
          ...
          <div className="flex flex-col gap-2 text-xs text-stone-500 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button
              ...
              disabled={!canSend}
              className={cn(
                "inline-flex min-h-11 items-center gap-2 rounded-full px-4 py-2 text-xs font-medium",
                canSend
