...
import { resolveApiBaseUrl } from "@/lib/runtime-api-base";
import type {
  AdminCategory,
  AdminLoginResponse,
  AdminMediaAsset,
...
function parseApiErrorMessage(status: number, detail: unknown): string {
  if (typeof detail === "string" && detail.trim()) {
    if (detail.includes("Unsupported mime type")) return "图片格式不支持，请使用 JPG / PNG / WEBP / GIF。";
    if (detail.includes("File too large")) return "图片上传失败，请稍后重试。";
    if (detail.includes("Missing content type")) return "未识别到图片类型，请重新选择图片后重试。";
    if (detail.includes("Illegal or mismatched media content")) return "图片文件异常，请重新选择原图上传。";
    return detail;
  }
...
async function adminFetch<T>(path: string, options: FetchOptions = {}): Promise<T> {
  ...
  const fallbackBaseUrls =
    typeof window === "undefined"
      ...
      : [
          apiBaseUrl,
          "/api/v1",
          `http://${window.location.hostname}:18000/api/v1`,
        ];
  ...
  if (!response) {
    ...
    throw new AdminApiError(`网络请求失败：${message}。请检查后端服务和 API 地址（当前：${apiBaseUrl}）。`, 0, lastError);
  }
...
export async function adminLogin(payload: { username: string; password: string }): Promise<AdminLoginResponse> {
...
export async function getAdminOrders(params?: Record<string, string | number | undefined>) {
  ...
  return adminFetch<AdminOrderListResponse>(`/admin/orders?${query.toString()}`);
}

export async function getAdminOrder(orderId: number) {
  return adminFetch<AdminOrder>(`/admin/orders/${orderId}`);
}

export async function updateAdminOrderStatus(orderId: number, status: "created" | "cancelled" | "fulfilled") {
  return adminFetch<AdminOrder>(`/admin/orders/${orderId}/status`, { method: "PATCH", body: { status } });
}
