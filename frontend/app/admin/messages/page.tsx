  ...
  return (
    <AdminPageShell title="联系留言" description="集中查看咨询、标记已读，并直接回复用户邮件。">
      <div className="grid gap-2 sm:flex">
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="搜索姓名/邮箱/主题" className="min-h-11 w-full rounded-lg border border-stone-300 px-3 py-2 text-sm sm:w-64" />
        <div className="grid grid-cols-3 gap-2 sm:flex sm:items-center">
          {[
            ...
            <button
              ...
              className={`min-h-11 rounded-lg border px-3 py-2 text-sm ${readFilter === option.key ? "border-stone-900 bg-stone-900 text-white" : "border-stone-300 text-stone-700"}`}
            >
              {option.label}
            </button>
          ))}
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_360px]">
        ...
        <div className="space-y-2 md:hidden">
          {data?.items.map((item) => (
            <button key={item.id} type="button" onClick={() => setFocus(item)} className="w-full rounded-2xl border border-stone-200 bg-white p-3 text-left">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium text-stone-800">{item.name}</p>
                <span className={`text-xs ${item.auto_reply_status === "replied" ? "text-emerald-600" : item.is_read ? "text-stone-500" : "text-amber-600"}`}>{renderStatus(item)}</span>
              </div>
              <p className="mt-1 line-clamp-1 text-sm text-stone-700">{item.subject}</p>
              <p className="mt-1 text-xs text-stone-500">{new Date(item.created_at).toLocaleString()}</p>
            </button>
          ))}
          {data?.items.length === 0 ? <p className="rounded-2xl border border-stone-200 bg-white px-4 py-8 text-center text-sm text-stone-500">暂无留言</p> : null}
        </div>
            ...
            <div className="space-y-3">
              ...
              <div className="rounded-xl border border-stone-200 bg-stone-50/80 p-3">
                ...
                <button
                  ...
                  className="mt-2 min-h-11 w-full rounded-lg bg-stone-900 px-3 py-2 text-sm text-white disabled:cursor-not-allowed disabled:opacity-55"
                >
                  {replySending ? "发送中..." : cooldownSeconds > 0 ? `${cooldownSeconds}s 后可再次发送` : "发送回复"}
                </button>
              </div>
              <button type="button" onClick={() => toggleRead(focus)} className="min-h-11 w-full rounded-lg border border-stone-300 px-3 py-2 text-sm">
