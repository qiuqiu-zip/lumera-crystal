...
export default function AdminPostsPage() {
  ...
  return (
    <AdminPageShell
      ...
      actions={
        <Link href="/admin/posts/new" className="inline-flex min-h-11 w-full items-center justify-center rounded-lg bg-stone-900 px-3 py-2 text-sm text-white sm:w-auto">
          新增博客
        </Link>
      }
    >
      <div className="grid gap-2 sm:flex">
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="搜索标题或 slug" className="min-h-11 w-full rounded-lg border border-stone-300 px-3 py-2 text-sm sm:w-64" />
        ...
        <button
          ...
          className="min-h-11 rounded-lg border border-stone-300 px-3 py-2 text-sm text-stone-700"
        >
          筛选
        </button>
      </div>
      ...
      <div className="space-y-2 md:hidden">
        {data?.items.map((item) => (
          <article key={item.id} className="rounded-2xl border border-stone-200 bg-white p-3">
            ...
            <div className="mt-3 grid grid-cols-2 gap-2">
              <Link href={`/admin/posts/${item.id}`} className="inline-flex min-h-10 items-center justify-center rounded-lg border border-stone-300 text-sm text-stone-700">编辑</Link>
        ...
        {data?.items.length === 0 ? <p className="rounded-2xl border border-stone-200 bg-white px-4 py-8 text-center text-sm text-stone-500">暂无博客</p> : null}
      </div>
