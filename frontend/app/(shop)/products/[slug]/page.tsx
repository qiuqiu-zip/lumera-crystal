...
export default async function ProductDetailPage({ params }: { params: { slug: string } }) {
  ...
  return (
    <div className="grid gap-5 xl:grid-cols-[1.15fr_0.85fr] xl:gap-8">
      ...
      <section className="space-y-4 sm:space-y-5 xl:sticky xl:top-24 xl:h-fit">
        <div className="space-y-4 rounded-3xl border border-ink/10 bg-gradient-to-br from-ink to-stone-800 p-4 text-ivory shadow-[0_18px_40px_-24px_rgba(20,20,20,0.9)] sm:p-6">
          <div className="flex flex-wrap items-center gap-2">
            <Badge>{product.intention}</Badge>
            {product.is_new ? <Badge>NEW</Badge> : null}
            <Badge>{product.status === "active" ? "在售" : "暂不可售"}</Badge>
          </div>
          <h1 className="font-serif text-2xl leading-tight sm:text-4xl">{product.name}</h1>
          <p className="text-sm text-ivory/75">{product.subtitle}</p>
          <div className="flex items-end justify-between gap-3">
            <p className="text-2xl font-semibold sm:text-3xl">¥{product.price}</p>
        ...
        <div className="grid grid-cols-1 gap-2 rounded-3xl border border-mist/70 bg-white/90 p-4 text-sm text-ink/75 shadow-[0_14px_32px_-26px_rgba(34,32,28,0.8)] sm:grid-cols-2 sm:gap-3 sm:p-5">
