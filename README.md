...
- `logistics-service`: standalone logistics service
- `docker-compose.yml`: postgres + logistics + app(FastAPI) + frontend
...
> `.env` 里可以改端口、数据库密码、`NEXT_PUBLIC_API_BASE_URL` 等配置。
...
- Frontend: `http://localhost:3000`
- Backend Docs: `http://localhost:8000/docs`
- Logistics Dashboard: `http://localhost:8010/dashboard`
...
`app` 容器默认启动时会自动执行 `alembic upgrade head`。  
如果你希望手动执行：
...
- 端口冲突：
  - 修改 `.env` 里的 `FRONTEND_PORT` / `BACKEND_PORT` / `POSTGRES_PORT` / `LOGISTICS_PORT`
- 数据库连接失败：
  - `docker compose ps` 查看 `postgres` 是否 `healthy`
  - `docker compose logs -f postgres app`
- 环境变量缺失：
  - 确认已执行 `cp .env.example .env`
  - 执行 `docker compose config` 检查展开后的配置
...
- max size: `20MB` per image file (`30MB` for gif/video, configurable)
